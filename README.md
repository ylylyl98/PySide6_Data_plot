...
## OpenCodex DeepSeek agent sync

The DeepSeek agent profiles are machine-local Codex configuration, so they are
not copied by Git. After pulling this repository on another PC, run the
following from the repository root in PowerShell:

```powershell
.\scripts\sync_opencodex_deepseek.ps1
```

The script backs up the two agent profiles and OpenCodex configuration, fixes
the canonical lowercase model IDs, removes the stale reasoning-summary
override, refreshes the OpenCodex/Codex catalog, and waits for readiness.
