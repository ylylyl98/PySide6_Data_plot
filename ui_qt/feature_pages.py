...
class FeatureTabsMixin:
    def _build_pl_tab(self) -> QWidget:
        ...
        source_grid.addWidget(self.pl_saved_results_btn, 2, 0, 1, 3)
    ...
    def _build_drr_tab(self) -> QWidget:
        ...
        self.drr_edit_measurements_btn.setMinimumHeight(UI_METRICS["input_h"])
        ...
        self.drr_clear_measurements_btn.setMinimumHeight(UI_METRICS["input_h"])
        ...
        self.drr_baseline_autofind_btn.setMinimumHeight(UI_METRICS["input_h"])
        ...
        self.drr_pin_baseline_chk = QCheckBox("Keep background")
        self.drr_pin_baseline_chk.setAccessibleName("Pin background when measurement changes")
        ...
        self.drr_baseline_combo.setMinimumHeight(UI_METRICS["input_h"])
        ...
        cmap.setMinimumHeight(UI_METRICS["input_h"])
    ...
    def _build_compare_tab(self) -> QWidget:
        ...
        # Keep the two assignment actions on one responsive row. Their text
        # remains readable at the minimum sidebar width while avoiding two
        # full-width buttons consuming the expanded section's vertical space.
        ...
        assignment_actions = QWidget()
        assignment_actions_layout = QHBoxLayout(assignment_actions)
        assignment_actions_layout.setContentsMargins(0, 0, 0, 0)
        assignment_actions_layout.setSpacing(6)
        for button in (self.cmp_auto_assign_btn, self.cmp_swap_kk_btn):
            button.setMinimumWidth(0)
            button.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            assignment_actions_layout.addWidget(button)
        assignment_layout.addWidget(assignment_actions)
        ...
        summary_row = QWidget()
        summary_layout = QVBoxLayout(summary_row)
        summary_layout.setContentsMargins(0, 0, 0, 0)
        summary_layout.setSpacing(2)
        summary_layout.addWidget(summary_label)
        summary_layout.addWidget(self.cmp_assignment_summary)
        assignment_layout.addWidget(summary_row)
        ...
        checks_row = QWidget()
        checks_grid = QGridLayout(checks_row)
        ...
        for index, key in enumerate(COMPARE_PANEL_ORDER):
            chk = QCheckBox(key)
        ...
        vp_form.setRowWrapPolicy(QFormLayout.WrapLongRows)
        ...
        self.cmp_vp_background_spin.setMinimumWidth(0)
        self.cmp_vp_background_spin.setMaximumWidth(180)
        self.cmp_vp_background_spin.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.cmp_vp_filename_preview.setMinimumWidth(0)
        self.cmp_vp_filename_preview.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
    ...
    def _build_mcd_tab(self) -> QWidget:
        ...
        self.mcd_clear_source_btn.setMinimumHeight(UI_METRICS["input_h"])
        ...
        form.addRow(self.mcd_auto_angles_chk)
    ...
    def _build_mcd_peak_shift_tab(self) -> QWidget:
        ...
        source_layout.addWidget(self.mcd_peak_source_selection_summary)
        ...
        for spin in (self.mcd_peak_prom_spin, self.mcd_peak_dist_spin, self.mcd_peak_smooth_spin, self.mcd_peak_jump_spin, self.mcd_peak_max_spin):
            ...
            # Keep compact detection inputs at Qt's native content width so a
            # short value such as ``0.030`` does not become a full-width field.
            spin.setFixedWidth(max(spin.minimumWidth(), spin.sizeHint().width()))
        ...
        cform.setRowWrapPolicy(QFormLayout.WrapLongRows)
    ...
    def _build_shg_tab(self) -> QWidget:
        ...
        integration_form.setRowWrapPolicy(QFormLayout.WrapLongRows)
        ...
        cosmic_form.setRowWrapPolicy(QFormLayout.WrapLongRows)
        ...
        angle_form.setRowWrapPolicy(QFormLayout.WrapLongRows)
        ...
        # The checkbox already carries the caption. Keeping it as a
        # standalone form field gives the long caption the full field column
        # at the compact sidebar width.
        angle_form.addRow(self.shg_include_failed_chk)
